...
parser = argparse.ArgumentParser(description='A PyTorch Implementation of Deep CNNs')
...
parser.add_argument('--dataset', default='PascalVOC', type=str, metavar='TRAINING_DATALOADER_CLASS',
                    help='Specify dataset class for loading (Default: PascalVOC)')
...
parser.add_argument('--train_root',  metavar="TRAINING_FILE",
                    help='training file')
...
parser.add_argument('--val_root', metavar="VALIDATION_FILE",
...
parser.add_argument('--year', default="2007", type=str,
                    help='Dataset year specific to PASCAL VOC dataset')
parser.add_argument('--download', default=False, help='Download the dataset')
...
def get_train_and_valid_data_loaders(block, args):
    ...
    train_dataset = args.dataset_class(args,
                                       root=args.train_root, is_training=True)
